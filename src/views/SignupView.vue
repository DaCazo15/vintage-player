...
const validateForm = () => {
  ...
  if (!name.value || name.value.trim().length < 2) {
    formError.value = 'Por favor, ingrese un nombre válido (mínimo 2 caracteres).'
    return false
  }
  ...
  <div class="flex-1 flex items-center justify-center p-6 select-none">
    <div class="w-full max-w-md bg-paper border-4 border-coffee rounded-3xl p-8 shadow-[8px_8px_0px_0px_rgba(92,61,46,1)]">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-5">
        <div>
          <label for="name" class="block font-roboto text-xs font-bold text-coffee uppercase tracking-wider mb-2">
            Nombre de Usuario
          </label>
          <input
            id="name"
      ...
      <div class="relative flex py-5 items-center">
        ...
        <span class="shrink mx-4 text-xs font-roboto text-coffee uppercase tracking-wider">o registrarse con</span>
        <div class="grow border-t border-coffee/30"></div>
      </div>
      ...
      <div class="text-center mt-8">
        <p class="font-roboto text-sm text-coffee">
          ...
          <router-link to="/login" class="font-bold text-petrol hover:text-terracotta underline ml-1">
            Inicia sesión aquí
          </router-link>
        </p>
      </div>
